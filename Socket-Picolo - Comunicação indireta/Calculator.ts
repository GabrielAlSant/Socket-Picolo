  ...
  const server = net.createServer(socket => {
    ...
    socket.on("data", data => {
      ...
      buffer = decodeStream(buffer, Envelope, msg => {
        ...
        console.log(`📥 Recebida amostra de "${origem}" → T:${a.temperatura} U:${a.umidade} I:${a.insolacao}`);
        ...
        console.log(
          `📊 Média atualizada (${origem}) → T:${media.temperatura.toFixed(2)} ` +
        ...
        console.log(`📤 Média enviada ao cliente (${origem})`);
      });
    });

    socket.on("close", () => {
      console.log(`❌ Cliente desconectou: ${socket.remoteAddress}:${socket.remotePort}`);
    });

    socket.on("error", err => {
      console.log(`⚠️ Erro no socket: ${err.message}`);
    });
  });

  server.listen(porta, () =>
    console.log(`🚀 Calculator RPC executando na porta ${porta}`)
  );
}
