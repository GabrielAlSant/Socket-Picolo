// Simula um bairro que gera leituras de sensores e as envia usando a arquitetura RPC.
...
const server: net.Server = net.createServer((socket: net.Socket) => {
  ...
  socket.on('data', (mensagem: Buffer) => {
    console.log('[RPC/Bairro] Cliente diz:', mensagem.toString('utf8'));
  });

  
  socket.on('end', () => {
    clearInterval(intervalId);
    console.log('[RPC/Bairro] Cliente finalizou a conexão');
  });

  
  socket.on('close', () => {
    clearInterval(intervalId);
    console.log('[RPC/Bairro] Conexão fechada');
  });
...
server.listen(porta, () => {
  console.log(`[RPC/Bairro] Servidor de bairro rodando na porta ${porta}`);
});
