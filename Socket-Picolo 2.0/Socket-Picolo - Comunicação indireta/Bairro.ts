// Simula um bairro publicando leituras de sensores para a camada de comunicação indireta.
...
const server: net.Server = net.createServer((socket: net.Socket) => {
  ...
  socket.on('data', (mensagem: Buffer) => {
    console.log('[Bairro.ts] Cliente diz:', mensagem.toString('utf8'));
  });

  
  socket.on('end', () => {
    clearInterval(intervalId);
    console.log('[Bairro.ts] Cliente finalizou a conexão');
  });

  
  socket.on('close', () => {
    clearInterval(intervalId);
    console.log('[Bairro.ts] Conexão fechada');
  });
...
server.listen(porta, () => {
  console.log(`[Bairro.ts] Servidor de bairro rodando na porta ${porta}`);
});
