// Serviço de cálculo que consome leituras da comunicação indireta e produz médias agregadas.
...
async function iniciar() {
  ...
  const server = net.createServer(socket => {
    console.log(`[Calculator.ts] 🔌 Nova conexão: ${socket.remoteAddress}:${socket.remotePort}`);
    ...
    socket.on("data", data => {
      ...
      buffer = decodeStream(buffer, Envelope, msg => {
        ...
        console.log(`[Calculator.ts] 📥 Recebida amostra de "${origem}" → T:${a.temperatura} U:${a.umidade} I:${a.insolacao}`);
        ...
        console.log(
          `[Calculator.ts] 📊 Média atualizada (${origem}) → T:${media.temperatura.toFixed(2)} ` +
        ...
        console.log(`[Calculator.ts] 📤 Média enviada ao cliente (${origem})`);
      });
    });

    socket.on("close", () => {
      console.log(`[Calculator.ts] ❌ Cliente desconectou: ${socket.remoteAddress}:${socket.remotePort}`);
    });

    socket.on("error", err => {
      console.log(`[Calculator.ts] ⚠️ Erro no socket: ${err.message}`);
    });
  });

  server.listen(porta, () =>
    console.log(`[Calculator.ts] 🚀 Calculator RPC executando na porta ${porta}`)
  );
}
