// Serviço cloud que consome resultados do cálculo na arquitetura de comunicação indireta e os grava/exibe.
...
async function iniciar() {
  ...
  const calc = net.createConnection({ host: "127.0.0.1", port: 4010 }, () =>
    console.log("[Cloud.ts] Cloud conectada ao Calculator (RPC)")
  );
  ...
  calc.on("data", data => {
    ...
    calcBuffer = decodeStream(calcBuffer, Envelope, msg => {
      ...
      const m = msg.media;
      console.log(
        `[Cloud.ts] MÉDIA (${m.origem}) ➜ T: ${m.temperatura.toFixed(2)} | U: ${m.umidade.toFixed(2)} | I: ${m.insolacao.toFixed(2)} (amostras: ${m.amostras})`
      );
    });
  });
  calc.on("error", err => console.log("[Cloud.ts] Erro Calc:", err.message));
      ...
      await consumersA.run({ 
        eachMessage: async ({ topic, partition, message }: EachMessagePayload) => {
          ...
          try {
            ...
            console.log(`[Cloud.ts] Kafka TopicA → Cloud:`, obj);
          } catch {
            console.log('[Cloud.ts] Mensagem Kafka inválida');
          }
        }
      });
      console.log('[Cloud.ts] Kafka Consumer conectado na Cloud');
    } catch (error) {
